...
var MarkerClusterer = function (_map, _markers, _config) {
  ...
  var clusterOptions = {map: _map, markers: _markers, renderer: interpolatedRenderer};
  if (_config) {
    var algorithmOptions = {};
    var maxZoom = parseInt(_config.maxZoom, 10);
    var radius = parseInt(_config.gridSize, 10);

    if (Number.isFinite(maxZoom) && maxZoom >= 0) {
      algorithmOptions.maxZoom = maxZoom;
    }
    if (Number.isFinite(radius) && radius > 0) {
      algorithmOptions.radius = radius;
    }

    clusterOptions.algorithm = new markerClusterer.SuperClusterAlgorithm(algorithmOptions);
  }
  if (_config && typeof _config.onClusterClick === "function") {
    clusterOptions.onClusterClick = function(event, cluster, clusterMap) {
      _config.onClusterClick(event, cluster, clusterMap);
      if (markerClusterer && typeof markerClusterer.defaultOnClusterClickHandler === "function") {
        markerClusterer.defaultOnClusterClickHandler(event, cluster, clusterMap);
      }
    };
  }
  return new markerClusterer.MarkerClusterer(clusterOptions);
};
