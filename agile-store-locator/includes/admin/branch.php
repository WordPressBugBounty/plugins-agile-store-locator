...
class Branch extends Base {
 ...
 /**
  ...
  public function get_store_list_edit() {
    ...
    // Select filter
    $select_filter = isset($_REQUEST['select_filter'])? sanitize_key($_REQUEST['select_filter']): null;
      ...
      for ( $i=0 ; $i < intval( $_REQUEST['iSortingCols'] ) ; $i++ )
      {
        if (isset($_REQUEST['iSortCol_'.$i]))
        {
          ...
          if(isset($acolumns[$col_index])) {
            $sOrder .= $acolumns[$col_index]." ".$sort_dir;
            break;
          }
        }
      }
  ...
  public function add_store_into_branch() {
    ...
    $toggle     = isset( $_REQUEST['toggle'])? sanitize_text_field($_REQUEST['toggle']): null;   
    $parent_id  = isset( $_REQUEST['parent_id'])? intval($_REQUEST['parent_id']): 0;   
