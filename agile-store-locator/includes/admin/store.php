...
class Store extends Base
{
    ...
    public function get_store_list()
    {
        ...
        $clause[] = ASL_PREFIX . "stores.lang = %s";
        $prepare_params[] = $this->lang;
    ...
    public function duplicate_store()
    {
        ...
        if ($result && $result[0]) {
            ...
            unset($result['id'], $result['created_on'], $result['updated_on']);
            ...
            if ($wpdb->insert(ASL_PREFIX . 'stores', $result)) {
                ...
                foreach ($s_categories as $_category) {
                    $wpdb->insert(
                        ...
                        ['store_id'=> $new_store_id, 'category_id'=>$_category->category_id],
                        ['%s', '%s']
                    );
                }
    ...
    public function update_store()
    {
        ...
        if ($update_id && is_numeric($update_id)) {
            ...
            $wpdb->update(ASL_PREFIX . 'stores', $store_data, ['id' => $update_id]);
    ...
    public static function approve_store($store_id)
    {
        ...
        if ($coordinates) {
            if ($wpdb->update(ASL_PREFIX . 'stores', ['pending' => null, 'lat' => $coordinates['lat'], 'lng' => $coordinates['lng']], ['id'=> $store->id])) {
                //  Send a notification
    ...
    public static function register_notification($form_data, $store_id)
    {
        ...
        if (isset($all_configs['admin_notify']) && $all_configs['admin_notify'] == '1') {
            ...
            if ($admin_email) {
                ...
                $locality = implode(', ', [$form_data['city'], $form_data['state'], $form_data['postal_code']]);
                ...
                foreach ($form_data as $key => $value) {
                    if (!in_array($key, ['title', 'description_2', 'country', 'is_disabled', 'logo_id', 'marker_id', 'custom', 'open_hours', 'ordr', 'pending', 'updated_on', 'lat', 'lng']) && $value) {
    ...
    public function schedule_the_store()
    {
        ...
        $data_params  = ['is_disabled' => $disable_switch];
        $where_clause = ['id' => $store_id];
